...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className, 'aria-invalid': ariaInvalid, ...props }, ref): JSX.Element => {
    return (
      <input id="auto-input" name="auto-input" ref={ref} aria-label="input" id={props.id ?? props.name} name={props.name ?? props.id} aria-label={props["aria-label"] ?? props.placeholder ?? "input"}
