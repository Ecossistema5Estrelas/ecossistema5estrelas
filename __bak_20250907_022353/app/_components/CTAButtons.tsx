...
const toneClass = (t:Btn["tone"]) =>
  t==="amber"?"btn-amber":t==="fuchsia"?"btn-fuchsia":"btn-emerald";
...
export default function CTAButtons(){
  ...
  return (
    <nav aria-label="Ações principais" className="grid gap-4 md:grid-cols-3">
      {items.map((b)=>(
        <a key={b.href} href={b.href} className={`btn-xxl ${toneClass(b.tone)}`} aria-label={b.label}>
